...
#define SI_PSZ(si, psz)                                                        \
  {                                                                            \
    si.cp = psz;                                                               \
    si.count = (FICL_COUNT)strlen(psz);                                        \
  }
...
#define CELLS_PER_WORD                                                         \
  ((sizeof(FICL_WORD) + nFICLNAME + sizeof(CELL)) / (sizeof(CELL)))
...
#define M_INNER_LOOP(pVM) ficlVmInnerLoop((ficlVm *)pVm);
...
FICL_WORD *dictAppendWord(FICL_DICT *pDict, char *name, FICL_CODE pCode,
                          UNS8 flags);
FICL_WORD *dictAppendWord2(FICL_DICT *pDict, STRINGINFO si, FICL_CODE pCode,
                           UNS8 flags);
...
void ficlAddPrecompiledParseStep(FICL_SYSTEM *pSys, char *name,
...
#define ficlInitInfo(x)                                                        \
  {                                                                            \
    memset((x), 0, sizeof(FICL_SYSTEM_INFO));                                  \
