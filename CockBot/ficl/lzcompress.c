...
#define NETWORK_ORDER(X)                                                       \
  ((((unsigned char *)X[0]) << 8) | (((unsigned char *)X[1])))

static int ficlLzCompareWindow(const unsigned char *window,
                               const unsigned char *buffer, int *offset,
                               unsigned char *next, int windowSize,
                               int bufferSize) {
  ...
  ** as the length (which won't work, max we can store is FICL_LZ_BUFFER_LENGTH
  *- 1)
  */
  bufferSize--;
...
void ficlLzEncodeHeaderField(unsigned char *data, unsigned int input,
                             int *byteOffset) {
  int i;
  ...
  if (input <= 252)
    ...
    for (i = 0; i < (length * 8); i++)
      ficlBitSet(data, bitsOffset++,
                 ficlBitGet((unsigned char *)&input, inputPosition++));
  }
}

int ficlLzCompress(const unsigned char *uncompressed, size_t uncompressedSize,
                   unsigned char **compressed_p, size_t *compressedSize_p) {
  ...
  *compressed_p = NULL;
  ...
  while (remaining > 0) {
    ...
    int length = ficlLzCompareWindow(window, buffer, &offset, &next,
                                     useWindowSize, bufferSize);
    if (length > 1) {
      /* phrase token */
      assert((length - FICL_LZ_MINIMUM_USEFUL_MATCH) <
             (1 << FICL_LZ_LENGTH_BITS));
      token = (1 << (FICL_LZ_PHRASE_BITS - 1)) |
              (offset << (FICL_LZ_PHRASE_BITS - FICL_LZ_TYPE_BITS -
                          FICL_LZ_OFFSET_BITS)) |
              ((length - FICL_LZ_MINIMUM_USEFUL_MATCH)
               << (FICL_LZ_PHRASE_BITS - FICL_LZ_TYPE_BITS -
                   FICL_LZ_OFFSET_BITS - FICL_LZ_LENGTH_BITS)) |
              next;
    ...
    for (i = 0; i < tokenLength; i++) {
      ...
      ficlBitSet(compressed, outputPosition,
                 ficlBitGet((unsigned char *)&token, inputPosition));
      outputPosition++;
    }
