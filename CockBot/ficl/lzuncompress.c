...
int ficlLzDecodeHeaderField(const unsigned char *data, int *byteOffset) {
  ...
  ficlBitGetString(((unsigned char *)&networkOrder), data, (*byteOffset) * 8,
...
int ficlLzUncompress(const unsigned char *compressed,
                     unsigned char **uncompressed_p,
  ...
  *uncompressed_p = NULL;
  ...
  while (inputPosition != bitstreamLength) {
    ...
    if (token) {
      /* phrase token */
      int offset = 0;
      ficlBitGetString((unsigned char *)&offset, compressed, inputPosition,
                       FICL_LZ_PHRASE_BITS - (1 + FICL_LZ_NEXT_BITS),
                       sizeof(offset) * 8);
      offset = ficlNetworkUnsigned32(offset);
      inputPosition += FICL_LZ_PHRASE_BITS - (1 + FICL_LZ_NEXT_BITS);

      length = (offset & ((1 << FICL_LZ_LENGTH_BITS) - 1)) +
    ...
    /* symbol token */
    *buffer = 0;
    ficlBitGetString(buffer++, compressed, inputPosition, FICL_LZ_NEXT_BITS,
                     sizeof(*buffer) * 8);
