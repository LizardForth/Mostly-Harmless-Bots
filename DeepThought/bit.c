...
void ficlBitGetString(unsigned char *destination, const unsigned char *source,
                      int offset, int count, int destAlignment) {
...
ficlUnsigned32 ficlNetworkUnsigned32(ficlUnsigned32 number) {
  ...
  return ((ficlUnsigned32)(ficlNetworkUnsigned16(pointer[0]) << 16)) |
         ficlNetworkUnsigned16(pointer[1]);
}
